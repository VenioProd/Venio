...
import type { SessionDuty, SessionLink, SessionReminder, SessionRemark, SessionWorkspacePayload } from './education'
...
export async function fetchCalendarEventWorkspace(occurrenceId: string): Promise<CalendarEventWorkspaceResponse> {
  ...
  return await apiFetch<CalendarEventWorkspaceResponse>(`/api/admin/education/calendar/workspace?${qs.toString()}`)
}
...
export async function updateCalendarEventWorkspace(
  ...
  return await apiFetch<CalendarEventWorkspaceResponse>(`/api/admin/education/calendar/workspace`, {
    method: 'PUT',
