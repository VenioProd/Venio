...
export default function InternKpi() {
  ...
  const handleExportCsv = () => {
    const headers = [
      'Nom', 'Email', 'Poste', 'Departement', 'Ecole', 'Tuteur',
      'Date debut', 'Date fin', 'Jours restants', 'Progression',
      'Rapports total', 'Valides', 'En attente', 'Brouillons',
      'Taux validation', 'Taches realisees', 'Fichiers joints',
      'Regularite (4 sem)', 'Derniere activite', 'Jours sans rapport',
    ]
    const rows = data.map((d) => [
      d.intern.name, d.intern.email, d.intern.poste, d.intern.departement,
      d.intern.ecole, d.intern.tuteur || '', fmtDate(d.intern.dateDebut),
      fmtDate(d.intern.dateFin), String(d.kpis.daysRemaining), `${d.kpis.progress}%`,
      String(d.kpis.totalReports), String(d.kpis.validated), String(d.kpis.pending),
      String(d.kpis.drafts), `${d.kpis.validationRate}%`, String(d.kpis.totalTaches),
      String(d.kpis.totalAttachments), `${d.kpis.regularite}%`,
  ...
  const handleExportPdf = async () => {
    ...
    doc.text(`Genere le ${now} — Filtre : ${filterStatus === 'ACTIF' ? 'Actifs' : filterStatus === 'TERMINE' ? 'Termines' : 'Tous'}`, 14, y)
    y += 14

    // Resume global
    const totalInterns = data.length
    const avgValidation = totalInterns > 0 ? Math.round(data.reduce((s, d) => s + d.kpis.validationRate, 0) / totalInterns) : 0
    const avgRegularite = totalInterns > 0 ? Math.round(data.reduce((s, d) => s + d.kpis.regularite, 0) / totalInterns) : 0
    ...
    doc.setFontSize(10)
    doc.setTextColor(60)
    doc.text(`Stagiaires : ${totalInterns}`, 14, y); y += 6
    doc.text(`Rapports totaux : ${totalReportsAll}`, 14, y); y += 6
    doc.text(`Taux validation moyen : ${avgValidation}%`, 14, y); y += 6
    doc.text(`Regularite moyenne (4 sem) : ${avgRegularite}%`, 14, y); y += 14
    ...
    data.forEach((d, idx) => {
      ...
      doc.setFontSize(9)
      doc.setTextColor(80)
      doc.text(`Poste : ${d.intern.poste}${d.intern.departement ? ' — ' + d.intern.departement : ''}`, 20, y); y += 5
      doc.text(`Stage : ${fmtDate(d.intern.dateDebut)} → ${fmtDate(d.intern.dateFin)} (${d.kpis.progress}%, ${d.kpis.daysRemaining}j restants)`, 20, y); y += 5
      if (d.intern.tuteur) { doc.text(`Tuteur : ${d.intern.tuteur}`, 20, y); y += 5 }
      if (d.intern.ecole) { doc.text(`Ecole : ${d.intern.ecole}`, 20, y); y += 5 }
      y += 2

      doc.setTextColor(40)
      doc.text(`Rapports : ${d.kpis.totalReports} total | ${d.kpis.validated} valides | ${d.kpis.pending} en attente | ${d.kpis.drafts} brouillons`, 20, y); y += 5
      doc.text(`Taux validation : ${d.kpis.validationRate}% | Regularite : ${d.kpis.regularite}%`, 20, y); y += 5
      doc.text(`Taches realisees : ${d.kpis.totalTaches} | Fichiers joints : ${d.kpis.totalAttachments}`, 20, y); y += 5
      doc.text(`Derniere activite : ${d.kpis.lastActivity ? fmtDate(d.kpis.lastActivity) : 'Aucune'}${d.kpis.daysSinceLastReport !== null ? ' (' + d.kpis.daysSinceLastReport + 'j)' : ''}`, 20, y); y += 5
      ...
      if (d.weeks.length > 0) {
        ...
        doc.setFontSize(9)
        doc.setTextColor(100)
        doc.text('Semaine          Rapports   Taches   Valides', 20, y); y += 5
        d.weeks.forEach((w) => {
          if (y > 275) { doc.addPage(); y = 20 }
          doc.setTextColor(60)
          doc.text(`${w.weekLabel}      ${w.reports}          ${w.taches}        ${w.validated}`, 20, y); y += 4.5
        })
      }
  ...
  if (loading) return <p style={{ color: 'rgba(255,255,255,0.4)', textAlign: 'center', padding: 40 }}>Chargement des KPIs...</p>
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button onClick={handleExportPdf} style={{
            padding: '8px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer',
            border: '1px solid rgba(139,92,246,0.3)', background: 'rgba(139,92,246,0.08)', color: '#8b5cf6',
          }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginRight: 6, verticalAlign: -2 }}>
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" y1="15" x2="12" y2="3" />
            </svg>
      ...
      <div className="ticket-stats" style={{ marginBottom: 20 }}>
        ...
        <div className="ticket-stat-card">
          <span style={{ color: '#8b5cf6', fontWeight: 700, fontSize: 22 }}>{avgValidation}%</span>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                {['Stagiaire', 'Poste', 'Rapports', 'Valides', 'Attente', 'Taux', 'Taches', 'Regularite', 'Dernier rapport', 'Progression'].map((h) => (
                  <th key={h} style={{ padding: '10px 8px', textAlign: 'left', color: 'rgba(255,255,255,0.5)', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {data.map((d) => {
                ...
                const alertLevel = k.daysSinceLastReport === null ? 'none' : k.daysSinceLastReport > 3 ? 'danger' : k.daysSinceLastReport > 1 ? 'warning' : 'ok'
                const colors = ['#0ea5e9', '#8b5cf6', '#22c55e', '#f59e0b', '#ef4444', '#ec4899']
                const avatarColor = colors[d.intern.name.charCodeAt(0) % colors.length]

                return (
                  <tr key={d.intern._id} onClick={() => setExpandedIntern(isExpanded ? null : d.intern._id)}
                  ...
                  >
                    <td style={{ padding: '12px 8px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <div style={{ width: 32, height: 32, borderRadius: '50%', background: avatarColor + '22', display: 'flex', alignItems: 'center', justifyContent: 'center', color: avatarColor, fontWeight: 700, fontSize: 13, flexShrink: 0 }}>
                          {d.intern.name.charAt(0).toUpperCase()}
                        </div>
                        <div>
              ...
              <div className="portal-card" style={{ marginTop: 8, marginBottom: 16 }}>
                <div style={{ padding: '16px 20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 20, marginBottom: 14, flexWrap: 'wrap' }}>
                    ...
                    {d.intern.tuteur && <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12 }}>Tuteur : {d.intern.tuteur}</span>}
                    <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 12 }}>Fichiers joints : {d.kpis.totalAttachments}</span>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
                    {d.weeks.map((w, i) => {
                      ...
                      return (
                        <div key={i} style={{ textAlign: 'center', padding: '12px 8px', borderRadius: 8, background: 'rgba(255,255,255,0.02)' }}>
                          ...
                          <div style={{ color: 'rgba(255,255,255,0.35)', fontSize: 10, marginTop: 2 }}>{w.taches} taches</div>
                          <div style={{ color: 'rgba(255,255,255,0.3)', fontSize: 9, marginTop: 4 }}>{w.weekLabel}</div>
                        </div>
                      )
                    })}
                  </div>
