...
export function NotesView({
  ...
  const { stage, restore, flush, discard, status: saveState, error: saveError } = useEducationAutosave()
  ...
  useEffect(() => {
    ...
    if (found) setActiveNote((current) => (current?._id === activeId ? current : restore('note', found._id, found)))
  }, [activeId, notes, restore])
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        if (!(await flush()) || cancelled) return
        ...
        setActiveNote(restore('note', note._id, note))
        setLoadError(null)
      } catch {
  ...
  }, [incomingOpenId, onCloseIncomingOpen, flush, restore])

  const persist = useCallback(
    (next: EducationNote) => {
      setActiveNote(next)
      setNotes((current) => current.map((note) => (note._id === next._id ? next : note)))
      stage('note', next._id, { title: next.title, blocks: next.blocks, pinned: next.pinned, archived: next.archived })
    },
    [stage],
  )
          ...
          {notes.length === 0 ? (
            ...
            notes.map((n) => (
              <div
                ...
                onClick={async () => {
                  if (await flush()) setActiveId(n._id)
                }}
              >
