...
interface Props {
  ...
  setReminderResult: (
    v: { sent: number; details?: { totalInterns?: number; today?: string; errors?: string[] } } | null,
  ) => void
        ...
        <div className="portal-card" style={{ marginTop: 16, marginBottom: 20 }}>
          ...
          >
            <div>
              ...
              <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12, marginLeft: 10 }}>
                Automatique tous les jours à 09h00 sur les jours de présence
              </span>
            </div>
            {isSuperAdmin && (
              <button
                onClick={async () => {
                  ...
                  try {
                    const res = await apiFetch<{ recipientsNotified: string[]; details?: any }>(
                      '/api/admin/interns/send-reminders',
                      { method: 'POST' },
                    )
          ...
          {reminderResult && (
            <div
              style={{
                padding: '10px 20px',
                background: reminderResult.sent > 0 ? 'rgba(34,197,94,0.08)' : 'rgba(255,200,0,0.08)',
                borderBottom: '1px solid rgba(255,255,255,0.06)',
                fontSize: 13,
              }}
            >
              ...
              {reminderResult.details && (
                <span style={{ color: 'rgba(255,255,255,0.4)', marginLeft: 10, fontSize: 11 }}>
                  ...
                  {(reminderResult.details.errors?.length ?? 0) > 0 &&
                    ` · erreurs: ${(reminderResult.details.errors ?? []).join(', ')}`}
                </span>
              )}
            </div>
          )}
          {reminderLogs.length === 0 ? (
            <div style={{ padding: '20px', color: 'rgba(255,255,255,0.3)', fontSize: 13, textAlign: 'center' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {dashboard.map((item: any) => {
                  ...
                  const daysSinceLogin = loginAt
                    ? Math.floor((Date.now() - new Date(loginAt).getTime()) / 86400000)
                    : null
                  const loginColor =
                    daysSinceLogin === null
                      ...
                      : daysSinceLogin === 0
                        ...
                        : daysSinceLogin <= 1
                          ...
                          : daysSinceLogin <= 3
                            ...
                            : '#ef4444'
                  const reportColor =
                    ds.daysSinceLastReport === null
                      ? 'rgba(255,255,255,0.3)'
                      : ds.daysSinceLastReport <= 1
                        ? '#22c55e'
                        : ds.daysSinceLastReport <= 3
                          ? '#f59e0b'
                          : '#ef4444'
                    ...
                    >
                      ...
                      <td style={{ padding: '10px 16px' }}>
                        ...
                        {loginAt && (
                          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 1 }}>
                            {new Date(loginAt).toLocaleString('fr-FR', {
                              day: '2-digit',
                              month: 'short',
                              hour: '2-digit',
                              minute: '2-digit',
                      ...
                      <td style={{ padding: '10px 16px' }}>
                        <span style={{ fontSize: 12, fontWeight: 600, color: reportColor }}>
                          {ds.lastActivity
                            ? ds.daysSinceLastReport === 0
                              ? "Aujourd'hui"
                              : ds.daysSinceLastReport === 1
                                ? 'Hier'
                                : `il y a ${ds.daysSinceLastReport}j`
                            : 'Aucun rapport'}
                        </span>
                        {ds.lastActivity && (
                          <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.3)', marginTop: 1 }}>
                            {formatDate(ds.lastActivity)}
                          </div>
                        )}
                      </td>
                    </tr>
                      ...
                      <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.06)' }}>
                        <div
                          style={{
                            ...
                            width: `${ds.progress}%`,
                            background:
                              ds.progress >= 90 ? '#ef4444' : ds.progress >= 70 ? '#f59e0b' : 'var(--primary)',
