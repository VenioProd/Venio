      ...
      <div className="ticket-attachments">
        {files.map((f, i) => (
          <div key={i} className="ticket-attachment">
            {isImage(f.mimetype) ? (
              <img
                src={`/api/admin/tickets/files/${f.filename}`}
                alt={f.originalName}
              ...
              <a
                href={`/api/admin/tickets/files/${f.filename}`}
                target="_blank"
    ...
    <div className={`ticket-card ${isExpanded ? 'expanded' : ''}`}>
      <div className="ticket-card-header" onClick={onToggleExpand}>
        <div className="ticket-card-left">
          <span className="ticket-category-badge" style={{ background: cat.color }}>
            {cat.label}
          </span>
        ...
        <div className="ticket-card-right">
          <span className="ticket-status-badge" style={{ color: st.color, borderColor: st.color }}>
            {st.label}
          </span>
          ...
          {ticket.replies.length > 0 && (
            <span className="ticket-reply-count">
              {ticket.replies.length} reponse{ticket.replies.length > 1 ? 's' : ''}
            </span>
          )}
          <svg
            width="14"
      ...
      {isExpanded && (
        <div className="ticket-card-body">
          <div className="ticket-meta">
            ...
            {ticket.archivedAt && (
              <span style={{ color: 'var(--ink-faint)' }}>Archive le {formatDate(ticket.archivedAt)}</span>
            )}
          </div>
                ...
                <div className="ticket-reply-form">
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <input
                      ...
                      style={{ display: 'none' }}
                      onChange={(e) => {
                        if (e.target.files) onReplyFilesChange([...replyFiles, ...Array.from(e.target.files!)])
                        e.target.value = ''
                      }}
                    />
