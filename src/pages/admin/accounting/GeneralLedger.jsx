...
export default function GeneralLedger() {
  ...
  // Chargement initial des listes (comptes actifs + exercices)
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      try {
            ...
            <div className="accounting-kpi-grid" style={{ marginBottom: 20 }}>
              <div className="accounting-kpi">
                ...
                <div className="value" style={{ fontSize: '1.1rem' }}>
                  ...
                  <div style={{ fontSize: '0.85rem', color: 'rgba(255,255,255,0.7)', fontWeight: 500, marginTop: 4 }}>
                    {account?.label}
                  </div>
                </div>
              </div>
                  ...
                  <tbody>
                    {movements.map((m) => (
                      <tr key={m._id}>
                        ...
                        <td>
                          <Link
                            to={`/admin/comptabilite/ecritures/${m._id}`}
                            className="code"
                          >
                            {m.entryNumber}
                          </Link>
