  ...
  return (
    ...
    >
      ...
      {payload.map((p) => (
        ...
        >
          <span>{p.name}</span>
          <span style={{ fontWeight: 600 }}>{formatEurDetail(p.value)}</span>
        </div>
      ))}
    </div>
  )
...
export default function AccountingDashboard() {
    ...
    async function load() {
      try {
        const [s, d, recentEntries] = await Promise.all([
          ...
          getAccountingDashboard().catch((err) => {
            if (!cancelled) {
              setDashboardError(err?.message || 'Impossible de charger les indicateurs')
            }
            return null
          }),
          listEntries({ limit: 8 }).catch(() => ({ entries: [], total: 0 })),
        ])
        if (cancelled) return
        setSettings(s)
        setDashboard(d)
  ...
  const kpi = dashboard?.kpi || EMPTY_KPI
    ...
    >
      ...
      <div className="accounting-kpi-grid">
        ...
        <div className="accounting-kpi">
          ...
          <div className="value">
            {loading ? '…' : formatEur(kpi.expensesYTD)}
          </div>
          <div
            style={{
              fontSize: '0.72rem',
        ...
        <div className="accounting-kpi">
          ...
          <div className="value">
            {loading ? '…' : formatEur(kpi.payables)}
          </div>
          <div
            style={{
              fontSize: '0.72rem',
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            >
              ...
              <Legend
                ...
                wrapperStyle={{ paddingTop: 16, fontSize: '0.85rem', color: 'rgba(255,255,255,0.7)' }}
              />
              <Bar
                dataKey="revenue"
                name="Produits"
      ...
      <section className="accounting-card">
        ...
        {loading ? (
          ...
          <div className="accounting-empty">
            Aucun produit comptabilisé sur l'exercice.
          </div>
        ) : (
