...
import { getEntry, validateEntry, deleteEntry, listAuditLogForEntity } from '../../../services/accounting'
      ...
      <section className="accounting-card">
        ...
        <div className="accounting-form">
          <div className="accounting-form-field">
            <label>Numéro</label>
            <div className="code" style={{ fontSize: '1rem' }}>{entry.entryNumber}</div>
          </div>
          <div className="accounting-form-field">
            <label>Journal</label>
          ...
          <div className="accounting-form-field">
            <label>Statut</label>
            <div>
              <span className={`accounting-badge ${entry.status.toLowerCase()}`}>{entry.status}</span>
            </div>
          </div>
          <div className="accounting-form-field">
            <label>Source</label>
          ...
          {entry.notes && (
            <div className="accounting-form-field full">
              <label>Notes</label>
              <div style={{ whiteSpace: 'pre-wrap', color: 'rgba(255,255,255,0.7)' }}>{entry.notes}</div>
            </div>
          )}
        </div>
      </section>
      ...
      <section className="accounting-card">
        ...
        <table className="accounting-table">
          ...
          <tbody>
            ...
            <tr>
              <td colSpan="2" style={{ fontWeight: 600, textTransform: 'uppercase', fontSize: '0.78rem', letterSpacing: '0.5px', color: 'rgba(255,255,255,0.55)' }}>
                Totaux
              </td>
              <td className="amount" style={{ fontWeight: 700 }}>{totalDebit.toFixed(2)} €</td>
              <td className="amount" style={{ fontWeight: 700 }}>{totalCredit.toFixed(2)} €</td>
              <td></td>
            </tr>
          </tbody>
        </table>
