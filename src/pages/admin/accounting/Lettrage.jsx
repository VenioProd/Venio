...
export default function Lettrage() {
  const [lettrableAccounts, setLettrableAccounts] = useState([])
  const [accountCode, setAccountCode] = useState('')

  const [unlettered, setUnlettered] = useState({ account: null, lines: [] })
  const [lettered, setLettered] = useState({ account: null, groups: [] })
  ...
  // Charger la liste des comptes lettrables
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      ...
      try {
        ...
        // Sélection par défaut : 411000 si présent, sinon premier compte
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setUnlettered(u || { account: null, lines: [] })
      setLettered(l || { account: null, groups: [] })
    } catch (err) {
  ...
  // Calculs des totaux sélectionnés
  ...
  // Suggestions : si une seule ligne est sélectionnée, on met en évidence
  // les autres lignes qui ont un montant opposé identique.
  const suggestionIds = useMemo(() => {
    if (selected.size !== 1) return new Set()
    const [onlyId] = Array.from(selected)
    const src = (unlettered.lines || []).find((l) => l._id === onlyId)
    if (!src) return new Set()
    const target = (Number(src.debit) || 0) > 0 ? 'credit' : 'debit'
    const amount = (Number(src.debit) || 0) || (Number(src.credit) || 0)
    if (!amount) return new Set()
    const ids = new Set()
  ...
  async function handleUnletter(code) {
          ...
          <section className="accounting-card" style={{ marginBottom: 16 }}>
            ...
            {showSuggestions && (
              <div className="accounting-message info" style={{ marginBottom: 12 }}>
                Suggestion : {suggestionIds.size} ligne(s) avec un montant opposé
                identique sont mises en évidence ci-dessous.
              </div>
            )}
            ...
            {(unlettered.lines || []).length === 0 ? (
              <div className="accounting-empty">
                ...
                <div className="hint">
                  Toutes les écritures de ce compte sont déjà lettrées ou il n'y a pas
                  encore d'écritures validées.
                </div>
              </div>
            ) : (
              <>
          ...
          <section className="accounting-card">
            ...
            {(lettered.groups || []).length === 0 ? (
              <div className="accounting-empty">
                ...
                <div className="hint">
                  Sélectionnez au moins 2 lignes ci-dessus puis cliquez sur « Lettrer
                  la sélection ».
                </div>
              </div>
            ) : (
                    ...
                    >
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                          ...
                          >
                            {g.lineCount} ligne(s) · Débit {formatEur(g.totalDebit)} ·
                            Crédit {formatEur(g.totalCredit)} ·{' '}
