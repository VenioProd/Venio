...
const ClientAccountList = () => {
  ...
  const [filters, setFilters] = useState<ClientListFilters>({ q: '', status: '', health: '', sort: 'updatedAt_desc', page: 1, limit: 12 })
    ...
    <div className="portal-container">
      <div className="portal-card">
          ...
          <div className="admin-actions portal-actions-reveal">
            <button
              ...
              onClick={() => {
                ...
                const rows = clients.map((c) => [c.name || '', c.companyName || '', c.email || '', c.serviceType || '', STATUS_LABELS[c.status] || c.status || '', c.healthStatus || '', c.ownerAdminId?.name || 'Non assigne'])
        ...
        <div className="portal-grid">
          <input className="portal-input" placeholder="Recherche nom, société, email" value={filters.q} onChange={(e: React.ChangeEvent<HTMLInputElement>) => updateFilter('q', e.target.value)} />
          ...
          <>
                  ...
                  return (
                    ...
                    >
                      <div className="client-card-header">
                        <div className="client-card-avatar" style={{ background: `linear-gradient(135deg, ${statusColor}28, ${statusColor}14)`, borderColor: `${statusColor}40`, color: statusColor }}>
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                      {['Client', 'Email', 'Statut', 'Santé', 'Service', 'Responsable'].map((h) => (
                        <th key={h} style={{ padding: '10px 16px', textAlign: 'left', color: 'rgba(255,255,255,0.4)', fontWeight: 600, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.5px' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        >
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              <div style={{ width: 32, height: 32, borderRadius: 8, background: `${statusColor}22`, border: `1px solid ${statusColor}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, fontWeight: 700, color: statusColor, flexShrink: 0 }}>
                                {(client.companyName || client.name || '?').charAt(0).toUpperCase()}
                              </div>
                              <div>
                                <div style={{ color: 'rgba(255,255,255,0.9)', fontWeight: 600 }}>{client.companyName || client.name}</div>
                                {client.companyName && client.companyName !== client.name && (
                          ...
                          {/* Email */}
                          <td style={{ padding: '12px 16px', color: 'rgba(255,255,255,0.45)', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            {client.email}
                          </td>
                          {/* Statut */}
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ padding: '3px 10px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: `${statusColor}18`, color: statusColor, border: `1px solid ${statusColor}35` }}>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                              <span style={{ width: 8, height: 8, borderRadius: '50%', background: healthColor, boxShadow: `0 0 6px ${healthColor}`, flexShrink: 0, display: 'inline-block' }} />
                              <span style={{ color: healthColor, fontSize: 12, fontWeight: 500 }}>{client.healthStatus || '—'}</span>
                            </div>
                          </td>
                          {/* Service */}
            ...
            <div style={{ marginTop: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="button" className="portal-button secondary" disabled={(meta.page || 1) <= 1} onClick={() => updateFilter('page', (meta.page || 1) - 1)}>Précédent</button>
                <button type="button" className="portal-button secondary" disabled={(meta.page || 1) >= (meta.totalPages || 1)} onClick={() => updateFilter('page', (meta.page || 1) + 1)}>Suivant</button>
              </div>
            </div>
          </>
        )}
