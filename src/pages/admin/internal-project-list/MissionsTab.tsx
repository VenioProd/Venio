...
export default function MissionsTab() {
  const {
    missions, missionsLoading, selectedMission, setSelectedMission, user, isSuperAdmin, projects, setShowMissionForm, handleMissionStatusUpdate, handleMissionProgressUpdate, handleMissionFileUpload, fileInputRefs, uploadingMission,
    ...
    <div style={{ marginTop: 20 }}>
      ...
      {isSuperAdmin && missions.length > 0 && (() => {
        ...
        byAssignee.forEach(entry => {
          entry.avgProgress = Math.round(entry.missions.reduce((sum: any, m: any) => sum + (m.progress ?? 0), 0) / entry.missions.length)
        })
        return (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                {(['Projet', 'Mission', ...(isSuperAdmin ? ['Assigné à'] : []), 'Statut', 'Progression', 'Fichiers', 'Deadline', '']).map((h: any, i: number) => (
