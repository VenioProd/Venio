...
interface MissionDrawerCtx {
  ...
  setDeliverableInputs: React.Dispatch<React.SetStateAction<Record<string, { title: string; description: string; assignedTo: string }>>>
...
export default function MissionDetailDrawer({ ctx }: Props) {
  const { missions, selectedMission, setSelectedMission, user, isSuperAdmin, expandedStep, setExpandedStep, stepAssigneeInputs, setStepAssigneeInputs, deliverableInputs, setDeliverableInputs, uploadingMission, fileInputRefs, missionStepInputs, setMissionStepInputs, handleParticipantUpdate, handleStepDescUpdate, handleMissionStatusUpdate, handleMissionToggleStep, handleMissionAddStep, handleMissionDateUpdate, handleDeliverableAdd, handleDeliverableToggle, handleDeliverableDelete, handleMissionFileUpload, handleMissionFileOpen, handleMissionFileDelete, handleMissionProgressUpdate, apiFetch, setMissions, showToast } = ctx
...
const Section = ({ icon, title, badge, children }: { icon: string; title: string; badge?: ReactNode; children: ReactNode }) => (
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', cursor: 'pointer' }} onClick={() => setExpandedStep(isOpen ? null : step._id)}>
                ...
                {step.waitingReview && !step.done && (
                  <>
                    ...
                    {isSuperAdmin && (
                      <button type="button" onClick={async e => { e.stopPropagation(); try { const data = (await apiFetch(`/api/admin/internal-projects/${m.internalProject?._id}/missions/${m._id}/steps/${step._id}/validate-step`, {method:'POST'})) as {mission:Mission}; setMissions(ms => ms.map(x => x._id===m._id ? data.mission : x)) } catch {} }}
