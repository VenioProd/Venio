  ...
  return (
    <div className="portal-container">
          ...
          <div className="confirm-modal confirm-modal--info" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true">
            ...
            <div className="confirm-modal__body">
              <div className="ticket-form-field">
                ...
                <textarea rows={4} placeholder="Ecrivez votre retour..." value={commentText} onChange={(e) => setCommentText(e.target.value)} autoFocus />
              </div>
            </div>
            <div className="confirm-modal__footer">
              <button className="confirm-modal__btn confirm-modal__btn--cancel" onClick={() => setCommentModal(null)}>Annuler</button>
              <button className="confirm-modal__btn confirm-modal__btn--confirm confirm-modal__btn--info" onClick={handleSubmitComment}>Envoyer</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="ticket-hero">
        <div className="ticket-hero-content">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 8 }}>
            ...
            <div>
              <h1 className="ticket-hero-title" style={{ margin: 0 }}>{intern.userId.name}</h1>
      ...
      <div className="ticket-stats" style={{ marginBottom: 20 }}>
        <StatCard value={String(stats.totalReports)} label="Rapports total" color="#0ea5e9" />
        <StatCard value={String(stats.reportsThisWeek)} label="Cette semaine" color="#22c55e" />
        <StatCard value={`${stats.validationRate}%`} label="Taux validation" color="#8b5cf6" />
        <StatCard value={String(stats.pendingReports)} label="En attente" color={stats.pendingReports > 0 ? '#f59e0b' : 'rgba(255,255,255,0.3)'} />
        <StatCard
          value={stats.daysSinceLastReport === null ? '—' : stats.daysSinceLastReport === 0 ? "Aujourd'hui" : `${stats.daysSinceLastReport}j`}
          label="Dernier rapport"
          color={stats.daysSinceLastReport === null ? 'rgba(255,255,255,0.3)' : stats.daysSinceLastReport > 3 ? '#ef4444' : stats.daysSinceLastReport > 1 ? '#f59e0b' : '#22c55e'}
        />
      </div>
      ...
      <div className="portal-card" style={{ marginBottom: 20 }}>
        <div style={{ padding: '20px 24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h3 style={{ margin: 0, color: '#0ea5e9', fontSize: 16 }}>Informations du stage</h3>
            {editing && (
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="ticket-new-btn" onClick={handleSave} disabled={submitting} style={{ fontSize: 12, padding: '6px 14px' }}>
                  {submitting ? 'Enregistrement...' : 'Enregistrer'}
                </button>
                <button className="ticket-back-btn" onClick={() => setEditing(false)} style={{ fontSize: 12, padding: '6px 14px' }}>Annuler</button>
              </div>
            )}
          </div>
          ...
          <div style={{ marginTop: 20 }}>
            ...
            <div style={{ height: 6, borderRadius: 3, background: 'rgba(255,255,255,0.06)' }}>
              <div style={{
                height: '100%', borderRadius: 3, width: `${stats.progress}%`,
                background: stats.progress >= 90 ? '#ef4444' : stats.progress >= 70 ? '#f59e0b' : '#0ea5e9',
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {(['cards', 'liste'] as const).map((v) => (
            <button key={v} onClick={() => setReportView(v)} style={{
              ...
              background: reportView === v ? '#0ea5e9' : 'rgba(255,255,255,0.06)',
              color: reportView === v ? '#fff' : 'rgba(255,255,255,0.5)', transition: 'all 0.2s',
            }}>
              {v === 'cards' ? 'Cards' : 'Liste'}
            </button>
          ))}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {reports.map((report) => {
                ...
                return (
                  <tr key={report._id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}
                    onMouseEnter={(e) => (e.currentTarget.style.background = 'rgba(255,255,255,0.03)')}
                    onMouseLeave={(e) => (e.currentTarget.style.background = 'transparent')}>
                    <td style={{ padding: '10px 8px', color: '#fff', fontWeight: 600, whiteSpace: 'nowrap' }}>{formatDate(report.date)}</td>
                    <td style={{ padding: '10px 8px' }}>
                      <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: sCfg.color + '22', color: sCfg.color }}>{sCfg.label}</span>
                    </td>
                    ...
                    <td style={{ padding: '10px 8px', color: 'rgba(255,255,255,0.6)', maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {report.contenu.length > 80 ? report.contenu.slice(0, 80) + '...' : report.contenu}
                    </td>
                    <td style={{ padding: '10px 8px' }}>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => setExpandedReport(expandedReport === report._id ? null : report._id)}
                          style={{ padding: '3px 8px', borderRadius: 4, fontSize: 11, cursor: 'pointer', border: '1px solid rgba(14,165,233,0.3)', background: 'rgba(14,165,233,0.08)', color: '#0ea5e9' }}>
                          {expandedReport === report._id ? 'Fermer' : 'Voir'}
                        </button>
                        {isSuperAdmin && report.status === 'SOUMIS' && (
                          <button onClick={() => handleValidateReport(report._id, 'VALIDE')}
                            style={{ padding: '3px 8px', borderRadius: 4, fontSize: 11, cursor: 'pointer', border: '1px solid rgba(34,197,94,0.3)', background: 'rgba(34,197,94,0.08)', color: '#22c55e' }}>
                            Valider
                          </button>
                        )}
                      </div>
            ...
            return (
              <div className="portal-card" style={{ marginTop: 8, borderLeft: `3px solid ${sCfg.color}` }}>
                <div style={{ padding: '16px 20px' }}>
                  ...
                  <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: 14, margin: '0 0 12px', whiteSpace: 'pre-wrap', lineHeight: 1.6 }}>{report.contenu}</p>
                  {report.taches.length > 0 && (
                    <ul style={{ margin: '0 0 12px', paddingLeft: 20 }}>
                      {report.taches.map((t, i) => <li key={i} style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 4 }}>{t}</li>)}
                    </ul>
                  )}
                  ...
                  {report.commentaireAdmin && (
                    <div style={{ padding: '10px 14px', borderRadius: 6, background: 'rgba(139,92,246,0.08)', borderLeft: '3px solid #8b5cf6', marginBottom: 12 }}>
              ...
              <div key={report._id} className="ticket-card" style={{ borderLeft: `3px solid ${sCfg.color}` }}>
                <div className="ticket-card-header" onClick={() => setExpandedReport(expanded ? null : report._id)} style={{ cursor: 'pointer' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    {report.attachments.length > 0 && <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: 12 }}>{report.attachments.length} fichier(s)</span>}
                    <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: sCfg.color + '22', color: sCfg.color }}>{sCfg.label}</span>
