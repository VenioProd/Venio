...
export default function InternalProjectDetail() {
  ...
  const handleParticipantUpdate = async (missionId: string, userId: string, progress?: number, status?: string) => {
    try {
                      ...
                      <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                        ...
                        {(m.participants || []).length > 0 && (
                          <div style={{ padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                            <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.5px', color: 'var(--text-secondary)', marginBottom: 10 }}>👥 Avancement par membre</div>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                              {(m.participants || []).map(p => {
                                const canEdit = isSuperAdmin || p.user?._id === user?._id
                                return (
                                  <div key={p._id} style={{ padding: '10px 12px', borderRadius: 8, background: p.user?._id === user?._id ? 'rgba(14,165,233,0.05)' : 'rgba(255,255,255,0.02)', border: `1px solid ${p.user?._id === user?._id ? 'rgba(14,165,233,0.15)' : 'rgba(255,255,255,0.05)'}` }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
                                      <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'rgba(165,180,207,0.12)', border: '1px solid rgba(165,180,207,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#a5b4cf', flexShrink: 0 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                      ...
                                      {canEdit ? (
                                        <input type="number" min={0} max={100} defaultValue={p.progress ?? 0} key={`${p._id}-${p.progress}`}
                                          onBlur={e => { const v = Math.min(100, Math.max(0, Number(e.target.value))); e.target.value = String(v); handleParticipantUpdate(m._id, p.user?._id, v, undefined) }}
                                    ...
                                    {canEdit && (
                                      <div style={{ display: 'flex', gap: 4, marginTop: 7, flexWrap: 'wrap' }}>
                                        {(['A_FAIRE', 'EN_COURS', 'TERMINE'] as const).map(v => (
                                          <button key={v} type="button" onClick={() => handleParticipantUpdate(m._id, p.user?._id, undefined, v)}
