 ...
 */
export function parseProjectGithubPatch(raw: unknown): Partial<DevProjectGithubConfig> | null | undefined {
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const members = Array.isArray(req.body?.members) ? Array.from(new Set(req.body.members.filter(isObjectId))) : []
      ...
      const githubConfig =
        githubPatch === undefined || githubPatch === null ? null : mergeGithubConfig(null, githubPatch)
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const [byStatusAgg, byPriorityAgg, byTypeAgg, total, completed14, completed7, created7, overdue, recentIssues] =
        await Promise.all([
          DevIssue.aggregate([
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (
        ...
        (DEV_PROJECT_STATUSES as readonly string[]).includes(req.body.status)
      ) {
...
router.delete(
  '/projects/:id',
  requirePermission(PERMISSIONS.MANAGE_DEV),
  sensitiveAction('DEV_PROJECT_DELETE'),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!isObjectId(req.params.id)) return res.status(400).json({ error: 'ID invalide' })
      const project = await DevProject.findById(req.params.id)
