...
router.post('/', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { title, emoji, cover, blocks, links, tags, pinned } = req.body
    ...
    const created = await EducationNote.create({
      ...
      blocks: blocksArr,
      markdown,
      links: Array.isArray(links) ? links.filter((l: { type?: string; refId?: string }) => l?.type && validId(l.refId)) : [],
...
router.patch('/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { title, emoji, cover, blocks, links, tags, pinned, archived } = req.body
