...
import { matchEventsToClasses, type ClassCandidate, type MatchedClass } from '../../../lib/appleCalendar/matching.js'
...
function getIcsUrl(): string | null {
  const raw = process.env.EDUCATION_APPLE_CALENDAR_ICS_URL || process.env.APPLE_CALENDAR_ICS_URL || ''
...
export function createCalendarRouter(deps: CalendarRouterDeps = {}) {
    ...
    try {
      ...
      if (!url) {
        return res.status(503).json({
          error: 'Aucune URL Apple Calendar configurée. Définir EDUCATION_APPLE_CALENDAR_ICS_URL côté serveur.',
          configured: false,
        })
      }
  ...
  r.get('/workspace', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({
        workspace: existing ? serializeWorkspace(existing) : emptyWorkspace(occurrenceId),
    ...
    try {
      ...
      if (
        ...
        (!validId(body.classId) ||
          !(await EducationClass.exists({ _id: body.classId, owner: req.user.id, deletedAt: null })))
      ) {
        return res.status(400).json({ error: 'Classe invalide ou inaccessible.' })
      }
      ...
      if (body.classId !== undefined) update.classId = body.classId || null
      ...
      await logActivity(req.user.id, req.user.id, 'calendarEventWorkspace', saved._id, 'UPDATE', {
        occurrenceId,
        kind: 'workspace',
      })
...
function serializeWorkspace(doc: InstanceType<typeof EducationCalendarEventWorkspace>): SerializedWorkspace {
