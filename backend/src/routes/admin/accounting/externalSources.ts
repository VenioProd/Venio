  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const source = await ExternalSource.create({
        ...
        defaultVatCollectedAccount: defaultVatCollectedAccount
          ? String(defaultVatCollectedAccount)
          : undefined,
        defaultVatDeductibleAccount: defaultVatDeductibleAccount
          ? String(defaultVatDeductibleAccount)
          : undefined,
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const { name, priority, enabled, conditions, mapping } = (req.body || {}) as Record<
        string,
        unknown
      >
      if (!name) {
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      const { status, from, to, page = '1', limit = '50' } = req.query as Record<
        string,
        string | undefined
      >
