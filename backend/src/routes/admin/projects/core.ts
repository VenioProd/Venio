  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (user.role !== 'SUPER_ADMIN') {
        ...
        const allProjectIds = [
          ...new Set([
            ...taskProjectIds.map(String),
            ...briefProjectIds.map(String),
            ...assignedProjectIds.map(String),
          ]),
        ]
...
function normalizeOptions(body: Record<string, any>): Record<string, any> {
  ...
  if (body.projectNumber !== undefined)
    opts.projectNumber = typeof body.projectNumber === 'string' ? body.projectNumber : ''
  ...
  if (body.internalNotes !== undefined)
    opts.internalNotes = typeof body.internalNotes === 'string' ? body.internalNotes : ''
  if (body.isArchived !== undefined) opts.isArchived = Boolean(body.isArchived)
  if (Array.isArray(body.tags))
    opts.tags = body.tags.filter((s: unknown) => typeof s === 'string' && (s as string).trim())
  ...
  if (body.billing && typeof body.billing === 'object') {
    ...
    opts.billing = {
      amountInvoiced:
        typeof bil.amountInvoiced === 'number' && !Number.isNaN(bil.amountInvoiced) ? bil.amountInvoiced : null,
      billingStatus: ['NON_FACTURE', 'PARTIEL', 'FACTURE'].includes(bil.billingStatus)
        ? bil.billingStatus
        : 'NON_FACTURE',
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (body.serviceTypes !== undefined)
        update.serviceTypes = options.serviceTypes !== undefined ? options.serviceTypes : body.serviceTypes
      if (body.deliverableTypes !== undefined)
        update.deliverableTypes =
          options.deliverableTypes !== undefined ? options.deliverableTypes : body.deliverableTypes
      if (body.deadlines !== undefined)
        update.deadlines = options.deadlines !== undefined ? options.deadlines : body.deadlines
      if (body.budget !== undefined) update.budget = options.budget !== undefined ? options.budget : body.budget
      if (body.startDate !== undefined)
        update.startDate =
          options.startDate !== undefined ? options.startDate : body.startDate ? new Date(body.startDate) : null
      if (body.endDate !== undefined)
        update.endDate = options.endDate !== undefined ? options.endDate : body.endDate ? new Date(body.endDate) : null
      if (body.deliveredAt !== undefined)
        update.deliveredAt =
          options.deliveredAt !== undefined ? options.deliveredAt : body.deliveredAt ? new Date(body.deliveredAt) : null
      if (body.projectNumber !== undefined)
        update.projectNumber = options.projectNumber !== undefined ? options.projectNumber : body.projectNumber
      if (body.priority !== undefined)
        update.priority = options.priority !== undefined ? options.priority : body.priority
      if (body.responsible !== undefined)
        update.responsible = options.responsible !== undefined ? options.responsible : body.responsible
      if (body.assignedTo !== undefined) update.assignedTo = body.assignedTo || null
      if (body.internalNotes !== undefined)
        update.internalNotes = options.internalNotes !== undefined ? options.internalNotes : body.internalNotes
      if (body.isArchived !== undefined)
        update.isArchived = options.isArchived !== undefined ? options.isArchived : body.isArchived
      ...
      if (body.reminderAt !== undefined)
        update.reminderAt =
          options.reminderAt !== undefined ? options.reminderAt : body.reminderAt ? new Date(body.reminderAt) : null
      ...
      if (oldProject) {
        if (update.status && update.status !== oldProject.status) {
          await logActivity({
            ...
            actor: req.user!.id,
            summary: `Statut changé de ${oldProject.status} à ${update.status}`,
            metadata: { from: oldProject.status, to: update.status },
          })
          ...
          if (update.status === 'TERMINE') {
            triggerAutomations(['project.closure_checklist'], {
          ...
          await logActivity({
            project: project._id,
            action: update.isArchived ? 'PROJECT_ARCHIVED' : 'PROJECT_UNARCHIVED',
            actor: req.user!.id,
            summary: update.isArchived ? 'Projet archivé' : 'Projet désarchivé',
          })
        } else {
          ...
          if (changedFields.length > 0) {
            await logActivity({
              ...
              actor: req.user!.id,
              summary: `Projet mis à jour (${changedFields.join(', ')})`,
            })
          }
        }
      }
...
router.delete(
  '/:id',
  requirePermission(PERMISSIONS.EDIT_PROJECTS),
  sensitiveAction('PROJECT_DELETE'),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await logActivity({
        ...
        actor: req.user!.id,
        summary: `Document uploadé : ${file.originalname}`,
