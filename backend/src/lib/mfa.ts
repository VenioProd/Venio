...
export async function createRecoveryCodes(count = 10): Promise<{ codes: string[]; hashes: string[] }> {
  const codes = Array.from({ length: count }, () => crypto.randomBytes(5).toString('hex').toUpperCase().match(/.{1,5}/g)!.join('-'))
  return { codes, hashes: await Promise.all(codes.map((code) => bcrypt.hash(code, 12))) }
}

export async function consumeRecoveryCode(hashes: string[], code: unknown): Promise<{ valid: boolean; hashes: string[] }> {
