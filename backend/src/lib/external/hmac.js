 ...
 * @param {string|Buffer} rawBody    Corps de la requête EXACT (bytes)
 * @param {string}        secret     webhookSecret de la source
 * @returns {string}                 "sha256=<hex>"
 */
export function computeSignature(timestamp, rawBody, secret) {
 ...
 *
 * @param {string|number} timestamp           valeur du header X-Venio-Timestamp
 * @param {string|Buffer} rawBody             corps brut tel que reçu (bytes)
 * @param {string}        secret              webhookSecret de la source
 * @param {string}        providedSignature   valeur du header X-Venio-Signature
 */
export function verifySignature(timestamp, rawBody, secret, providedSignature) {
