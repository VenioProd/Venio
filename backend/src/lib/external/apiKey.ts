 ...
 * @returns plain  : la clé en clair (à remettre à l'utilisateur une seule fois)
 ...
 *
 * @param plain  Clé reçue dans le header X-Api-Key
 * @param hash   Hash bcrypt persisté (ExternalSource.apiKeyHash)
 */
export async function verifyApiKey(plain: unknown, hash: unknown): Promise<boolean> {
