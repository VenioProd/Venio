...
function drawHorizontalLine(pdf, y, color = '#cccccc') {
  pdf.save()
...
function renderHeader(pdf, doc, settings) {
...
function renderClientBlock(pdf, client, project, yStart) {
  pdf.save()
...
function renderLinesTable(pdf, lines, currency, yStart) {
  // En-tête
  pdf.save()
...
function renderTotalsAndVat(pdf, doc, currency, yStart) {
...
function renderPaymentBlock(pdf, doc, settings, yStart) {
  const isQuote = doc.type === 'QUOTE'
  let y = yStart + 20
  pdf.fillColor('#666').font('Helvetica-Bold').fontSize(8)
...
function renderFooter(pdf, settings) {
  const y = 760
 ...
 *
 * @param {object} doc           BillingDocument (lean)
 * @param {object} client        User (client) avec name, email, companyName, address...
 * @param {object} project       Project lean
 * @param {string} storagePath   Chemin relatif où sauvegarder le PDF
 * @param {object} [opts]
 * @param {object} [opts.settings]  CompanySettings (pour éviter une 2e requête)
 * @returns {Promise<string>}    Chemin absolu du PDF généré
 */
export async function generateBillingPdf(doc, client, project, storagePath, opts = {}) {
  const absolutePath = path.resolve(process.cwd(), storagePath)
  await fsp.mkdir(path.dirname(absolutePath), { recursive: true })

  let settings = opts.settings
  if (!settings) {
    settings = await CompanySettings.findOne({ singletonKey: 'MAIN' }).lean().catch(() => null)
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    // Embed Factur-X (profil MINIMUM) si activé et si c'est une facture.
    ...
    if (
      ...
      try {
        ...
        pdf.file(Buffer.from(xml, 'utf-8'), {
          name: 'factur-x.xml',
          type: 'text/xml',
        ...
        // Recovery silencieux conforme au style codebase.
        // eslint-disable-next-line no-console
        console.error('Factur-X embed failed:', err.message)
      }
    }
