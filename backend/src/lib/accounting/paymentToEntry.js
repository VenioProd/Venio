 ...
 */
async function resolveBankAccount({ accountCode } = {}) {
  if (accountCode) {
 ...
 * Idempotent.
 */
export async function createPaymentEntryFromBilling(billingDocOrId, options = {}) {
  const doc =
    typeof billingDocOrId === 'object' && billingDocOrId._id
      ? billingDocOrId
  ...
  if (doc.type !== 'INVOICE') {
    const err = new Error('Seules les factures peuvent générer une écriture de paiement')
    err.status = 400
    throw err
  }

  const idempotencyKey = buildBillingIdempotencyKey(doc._id, 'payment')
  ...
  if (!bankAccount || !customerAccount) {
    const err = new Error('Comptes 512 ou 411 manquants — initialisez le plan comptable')
    err.status = 500
    throw err
  }
  ...
  if (totalTtc <= 0) {
    const err = new Error('Montant de paiement nul')
    err.status = 400
    throw err
  }

  // Lettrage commun avec la vente si elle existe
  const saleIdempotencyKey = buildBillingIdempotencyKey(doc._id, 'sale')
 ...
 */
export async function getDefaultBankAccountCode() {
