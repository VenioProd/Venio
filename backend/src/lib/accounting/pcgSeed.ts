...
import type { AccountType, JournalType } from '../../types/enums.js'
...
interface VatRateSeed {
  code: 'NORMAL' | 'INTERMEDIAIRE' | 'REDUIT' | 'SUPER_REDUIT' | 'EXONERE'
  label: string
  rate: number
...
const PCG_AGENCE: PcgAccountSeed[] = [
  // ---- Classe 1 : Capitaux ----
...
const DEFAULT_JOURNALS: JournalSeed[] = [
...
const DEFAULT_VAT_RATES: VatRateSeed[] = [
  {
    code: 'NORMAL',
...
interface SeedDeps {
  ChartOfAccount: Model<IChartOfAccount>
  Journal: Model<IJournal>
  VatRate: Model<IVatRate>
}
 ...
 */
export async function seedAccountingDefaults({ ChartOfAccount, Journal, VatRate }: SeedDeps): Promise<SeedResult> {
  const created: SeedResult = { accounts: 0, journals: 0, vatRates: 0 }
