 ...
 * Garantit "0.00" même si entrée null/undefined.
 *
 * @param {number} n
 * @returns {string}
 */
...
/**
 * Agrège les lignes de facture par taux de TVA pour reconstituer
 * TaxBasisTotalAmount / TaxTotalAmount.
 *
 * @param {Array<{ total: number, taxRate: number }>} lines
 * @returns {{ basisTotal: number, taxTotal: number, byRate: Array<{ rate: number, basis: number, tax: number }> }}
 */
 ...
 *
 * @param {object} params
 * @param {object} params.doc       BillingDocument (lean) — INVOICE (type 380)
 *                                  ou avoir (CREDIT_NOTE) → on déduit du total négatif.
 * @param {object} params.client    User
 * @param {object} params.project   Project
 * @param {object} params.settings  CompanySettings du vendeur
 * @returns {string}                XML UTF-8 complet (avec déclaration XML)
 */
export function buildFacturXMinimum({ doc, client, project, settings }) {
  ...
  const typeCode = grandTotal < 0 ? '381' : '380' // 381 = avoir, 380 = facture commerciale
  ...
  // Code pays ISO 3166-1 alpha-2 : on tente une déduction simple.
...
/**
 * Conversion grossière nom de pays → code ISO 3166-1 alpha-2 pour les cas
 * fréquents. Renvoie 'FR' par défaut.
 *
 * @param {string} name
 * @returns {string}
 */
function countryToIsoCode(name) {
  if (!name) return 'FR'
