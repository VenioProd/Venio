...
describe('lib/education/notion — extractNotionId', () => {
  ...
  it('extracts a flat 32-hex id from a Notion URL', () => {
    const id = extractNotionId('https://www.notion.so/workspace/My-Page-1234abcd5678ef901234abcd5678ef90')
...
describe('lib/education/notion — rich text', () => {
  it('joins plain_text across items', () => {
    expect(notionRichTextToPlainText([{ plain_text: 'Hello ' }, { plain_text: 'World' }])).toBe('Hello World')
  })

  it('falls back to text.content when plain_text is missing', () => {
    expect(notionRichTextToPlainText([{ text: { content: 'fallback' } }])).toBe('fallback')
  })
