...
describe('education documents — VENIO-46', () => {
  ...
  it('refuse si ni fichier ni url', async () => {
    await request(app).post('/api/admin/education/documents').field('title', 'rien').expect(400)
  })
  ...
  it('liste avec recherche + filtres et renvoie les compteurs par catégorie', async () => {
    ...
    const search = await request(app).get('/api/admin/education/documents?search=Sujet').expect(200)
    expect(search.body.documents).toHaveLength(1)
    expect(search.body.documents[0].title).toBe('Sujet 1')

    // Filtre catégorie
    const cat = await request(app).get('/api/admin/education/documents?category=assignment_correction').expect(200)
    expect(cat.body.documents).toHaveLength(1)
    expect(cat.body.documents[0].parentType).toBe('session')

    // Filtre école
    const school = await request(app).get('/api/admin/education/documents?school=EMA').expect(200)
    expect(school.body.documents).toHaveLength(2)

    // Filtre tag exact
    const tag = await request(app).get('/api/admin/education/documents?tag=examen').expect(200)
  ...
  it("download sur un document url-only redirige vers l'URL externe", async () => {
    ...
    const res = await request(app).get(`/api/admin/education/documents/${id}/download`).expect(302)
