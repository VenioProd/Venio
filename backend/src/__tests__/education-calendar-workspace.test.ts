...
describe('VENIO-44 — workspace événement calendrier', () => {
  ...
  it('rejette une requête sans occurrenceId', async () => {
    await request(app).get('/api/admin/education/calendar/workspace').expect(400)
    await request(app).put('/api/admin/education/calendar/workspace').send({ notes: 'sans id' }).expect(400)
  })
  ...
  it('rattache la fiche à une EducationClass quand classId est fourni', async () => {
    const klass = await request(app).post('/api/admin/education/classes').send({ name: 'EMA B3 Marketing' }).expect(201)
    const r = await request(app)
      .put('/api/admin/education/calendar/workspace')
      .send({
        occurrenceId: OCCURRENCE,
  ...
  it('persiste les remarques avec id stable et filtre les entrées vides', async () => {
    const r = await request(app)
      ...
      .send({
        ...
        reminders: [{ label: 'Apporter le rétroprojecteur', dueAt: '2026-06-01T08:00:00.000Z' }],
      })
      .expect(200)
...
it('preserves a class association on a notes-only patch and rejects a foreign class', async () => {
  const klass = await request(app).post('/api/admin/education/classes').send({ name: 'Scoped class' }).expect(201)
  await request(app)
    .put('/api/admin/education/calendar/workspace')
    .send({ occurrenceId: OCCURRENCE, classId: klass.body.class._id })
    .expect(200)
  const updated = await request(app)
    .put('/api/admin/education/calendar/workspace')
    .send({ occurrenceId: OCCURRENCE, notes: 'Latest' })
    .expect(200)
  expect(updated.body.workspace.classId).toBe(klass.body.class._id)
  currentOwner = OTHER_OWNER_ID
  await request(app)
    .put('/api/admin/education/calendar/workspace')
    .send({ occurrenceId: OCCURRENCE, classId: klass.body.class._id })
