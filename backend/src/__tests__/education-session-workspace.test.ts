...
async function seedSession() {
  const klass = await request(app).post('/api/admin/education/classes').send({ name: 'Promo X' }).expect(201)
  ...
  it('persiste un workspace complet via PUT /:id/workspace', async () => {
    ...
    const r = await request(app)
      ...
      .send({
        ...
        duties: [{ label: 'Lecture chapitre 4 pour la prochaine séance', done: false }],
      })
      .expect(200)
