...
END:VCALENDAR`
    const { app: localApp } = makeApp(ics, {
      classes: [{ _id: 'class-ema', name: 'EMA B3 Marketing', school: 'EMA', level: 'B3', program: 'Marketing' }],
    })
    const r = await request(localApp).get('/api/admin/education/calendar/upcoming').query({ days: 7 }).expect(200)
    expect(r.body.configured).toBe(true)
  ...
  it('GET /upcoming rejects an invalid days param', async () => {
    ...
    await request(localApp).get('/api/admin/education/calendar/upcoming').query({ days: '999' }).expect(400)
  })

  it('GET /upcoming returns 503 when calendar is unconfigured', async () => {
    const { app: noUrlApp } = makeApp(ICS_SAMPLE, { url: null })
    const r = await request(noUrlApp).get('/api/admin/education/calendar/upcoming').expect(503)
    expect(r.body.configured).toBe(false)
