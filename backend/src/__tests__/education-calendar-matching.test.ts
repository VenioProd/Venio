...
import { matchEventToClass, matchEventsToClasses, type ClassCandidate } from '../lib/appleCalendar/matching.js'
...
describe('appleCalendar / matching', () => {
  ...
  it('matches exactly when the class name appears verbatim in the event', () => {
    const m = matchEventToClass({ title: 'EMA B3 Marketing — séance SEO', location: 'EMA Paris' }, CLASSES)
  ...
  it('returns null when nothing meaningful overlaps', () => {
    const m = matchEventToClass({ title: 'Rendez-vous dentiste', location: 'Paris' }, CLASSES)
    expect(m).toBeNull()
  })

  it('handles accents and casing in titles', () => {
    const m = matchEventToClass({ title: 'ÉCOLE — Séance UX/UI ESVE M1' }, CLASSES)
    expect(m).not.toBeNull()
  ...
  it('batches via matchEventsToClasses preserving order', () => {
    const events = [{ title: 'EMA B3 Marketing' }, { title: 'Rendez-vous dentiste' }, { title: 'ESVE M1 UX/UI' }]
