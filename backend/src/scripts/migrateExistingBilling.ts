 ...
 *
 * Idempotent : utilise les idempotencyKey de createEntry. Safe à relancer.
 *
 * Usage :
 *   npm run accounting:migrate-billing
 *   npm run accounting:migrate-billing:dry
 */
...
async function main(): Promise<void> {
  ...
  for (const invoice of invoices) {
    ...
    try {
      const r = await createSaleEntryFromBilling(invoice as unknown as IBillingDocument)
      if (r.alreadyExisted) {
        saleSkipped += 1
      } else {
        saleCreated += 1
      ...
      errors += 1
      console.error(`  ✗ ${invoice.number} (VE) : ${(err as Error).message}`)
      continue
    }

    if (invoice.status === 'PAID') {
      try {
        const r = await createPaymentEntryFromBilling(invoice as unknown as IBillingDocument)
        if (r.alreadyExisted) {
          paymentSkipped += 1
        } else {
          paymentCreated += 1
        ...
        errors += 1
        console.error(`  ✗ ${invoice.number} (BQ) : ${(err as Error).message}`)
      }
    }
  }
