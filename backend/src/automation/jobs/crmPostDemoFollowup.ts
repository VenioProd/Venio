// ─────────────────────────────────────────────────────────────
// crm.post_demo_followup
// Rappel de suivi après démo (3 jours)
// ─────────────────────────────────────────────────────────────
...
const definition: AutomationDefinition = {
  key: 'crm.post_demo_followup',
  title: 'Relance post-démo',
  domain: 'crm',
  triggerType: 'cron',
  schedule: '09:00',
  channels: ['in_app'],
  ...
  buildIdempotencyKey: (ctx) => `crm.post_demo:${ctx.dateKey}`,
  ...
  execute: async (ctx: AutomationContext): Promise<AutomationResult> => {
    ...
    const leads = await Lead.find({
      status: 'DEMO',
      statusChangedAt: { $lt: threeDaysAgo },
      isArchived: { $ne: true },
      assignedTo: { $ne: null },
    ...
    for (const lead of leads) {
      ...
      const daysSinceDemo = Math.floor((ctx.now.getTime() - new Date(lead.statusChangedAt).getTime()) / 86400_000)

      await createNotification({
        recipient: assigneeId,
        type: 'CRM_FOLLOWUP' as never,
        title: `Suivi post-démo : ${lead.company || lead.contactName}`,
        message: `La démo avec "${lead.company || lead.contactName}" date de ${daysSinceDemo}j. Pensez à relancer !`,
        link: '/admin/crm',
      })

      actionsExecuted.push(`post_demo:lead:${(lead._id as object).toString()}`)
