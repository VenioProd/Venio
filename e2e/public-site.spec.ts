...
test('contact form uses the deterministic test transport and records a success state', async ({ page }) => {
  ...
  await expect(page.locator('input:not([type="hidden"]):not([aria-label])')).toHaveCount(0)
  await expect(page.locator('select:not([aria-label]), textarea:not([aria-label])')).toHaveCount(0)
  ...
  await page.getByPlaceholder('Réponse').fill('5')
  await page.getByRole('button', { name: 'Envoyer' }).click()
  await expect(page.getByText('Votre message a été envoyé. On vous répond sous 48h.')).toBeVisible()
